...
def test_IMM_mat():  # noqa
    ...
    state_mat1 = np.array([[1, 0, dt, 0], [0, 1, 0, dt], [0, 0, 1, 0], [0, 0, 0, 1]])

    ta = dt * np.pi / np.sqrt(2)
    state_mat2 = np.array(
        [
            [1, 0, dt * np.cos(ta), dt * np.sin(ta)],
            [0, 1, dt * -np.sin(ta), dt * np.cos(ta)],
            [0, 0, np.cos(ta), np.sin(ta)],
            [0, 0, -np.sin(ta), np.cos(ta)],
        ]
    )
