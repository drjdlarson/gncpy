"""LQR control validation for ComplexMultirotor with TILTED motors.

This test validates the ComplexMultirotor dynamics with motors tilted outward
by 15 degrees. This tests the arbitrary thrust direction capability while
still being controllable with LQR (tilt is small enough for stability).

Expected behavior:
- Tilted motors reduce effective vertical thrust (cos(15 deg) ~= 96.6% efficiency)
- Creates passive stability (tilted thrust vectors resist roll/pitch)
- Should still track to target waypoint with LQR control
- Error may be slightly larger than vertical config due to coupling
"""
...
# ============================================================================
print("Initializing ComplexMultirotor with TILTED motors (15 deg outward)...")
config_file = Path(__file__).parent / "complex_quad_tilted_config.yaml"
...
# Calculate hover motor command (needs more thrust due to tilted motors)
# Vertical component: cos(15 deg) ~= 0.9659
mass = vehicle.vehicle.params.mass.mass_kg
gravity = 9.81
tilt_efficiency = 0.9659  # cos(15 deg)
hover_thrust_per_motor = (
    mass * gravity / vehicle.vehicle.params.motor.num_motors
) / tilt_efficiency
...
hover_cmd = float(valid_roots[0].real) if len(valid_roots) > 0 else 0.75
...
print(f"Note: Higher than vertical config ({0.700:.3f}) due to tilt efficiency")
...
axes[0, 0].legend()
axes[0, 0].grid(True)
axes[0, 0].set_title("NED Position (Tilted Motors 15 deg)")
...
plt.tight_layout()
output_file = Path(__file__).parent / "lqr_complex_tilted_results.png"
plt.savefig(output_file, dpi=150)
print(f"Plot saved: {output_file}")

print("\n" + "=" * 70)
print("Validation complete!")
print(f"Configuration: Motors tilted 15 deg outward")
print(f"Expected: Slightly higher error than vertical config due to coupling")
print(f"Typical range: 0.001-0.01m depending on LQR tuning")
print("=" * 70)
