def linear_inf_horizon():
    ...
    dynObj.control_model = lambda _t, _x, *_args: np.array(
        [[0, 0], [0, 0], [1, 0], [0, 1]]
    )
    state_args = (dt,)
    ...
    u, cost, state_traj, ctrl_signal, feedback_gain = lqr.calculate_control(
        cur_time,
        cur_state,
        time_horizon,
        end_state=end_state,
...
def make_plot(x_data, y_data, x_lbl, y_lbl, ttl):
    ...
    fig.axes[0].scatter(x_data[-1], y_data[-1], marker="x", color="r")
