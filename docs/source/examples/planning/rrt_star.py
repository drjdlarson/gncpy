...
def elqrrrtstar():
    ...
    controller.set_cost_model(Q=Q, R=R, non_quadratic_fun=non_quadratic_cost, quad_modifier=quad_modifier)
    ...
    elqrRRTStar = ExtendedLQRRRTStar(rng=rng, sampling_fun=sampling_fun)
...
def run():
    ...
    fout = os.path.join(os.path.dirname(__file__), "elqrrrtstar.gif")
    if not os.path.isfile(fout):
        frame_list = elqrrrtstar()
        frame_list[0].save(
            fout,
            save_all=True,
            append_images=frame_list[1:],
