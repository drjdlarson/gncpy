"""Implements a simple multi-rotor dynamics model with quaternion attitude representation."""
...
class VehicleQuat(SimpleVehicle):
    """Implements a simple multirotor vehicle with quaternion attitude representation.

    This extends the simple vehicle to use quaternions instead of Euler angles,
    avoiding gimbal lock issues while maintaining compatibility with the simple
    multirotor motor configuration (vertical thrust only).
...
class SimpleMultirotorQuat(SimpleMultirotor):
    """Implements a simple multi-rotor with quaternion attitude representation.

    This class extends SimpleMultirotor to use quaternions instead of Euler angles,
    avoiding gimbal lock while maintaining the simple vertical-thrust motor model.
    ...
    vehicle : :class:`.VehicleQuat`
        Vehicle class with quaternion attitude dynamics.
    """
    ...
    def __init__(
        ...
        **kwargs : dict
        ...
        """
        # Call parent's parent to skip SimpleMultirotor.__init__
        from gncpy.dynamics.basic.nonlinear_dynamics_base import NonlinearDynamicsBase
        import pathlib
        import os
        from ruamel.yaml import YAML
        from gncpy.dynamics.aircraft.simple_multirotor import Effector, Environment

        NonlinearDynamicsBase.__init__(self, dt=dt, **kwargs)
        ...
        # Create VehicleQuat instead of simple Vehicle
        self.vehicle = VehicleQuat(v_params)
