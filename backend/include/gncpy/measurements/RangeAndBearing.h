...
protected:
    std::vector<std::function<T (const matrix::Vector<T>&)>> getMeasFuncLst(const std::unique_ptr<MeasParams>& params) const override {
...
private:
    static T range (const matrix::Vector<T>& state, const std::unique_ptr<MeasParams>& params=std::make_unique<MeasParams>()) {
        if (!params) {
            throw BadParams("Range and Bearing requires parameters.");
        }
        if (!utilities::instanceof<RangeBearingParams>(params.get())) {
            throw BadParams("params type must be RangeBearingParams.");
        }
        auto ptr = dynamic_cast<RangeBearingParams*>(params.get());

        return sqrt(state(ptr->xInd) * state(ptr->xInd) + state(ptr->yInd) * state(ptr->yInd));
    }
    static T bearing (const matrix::Vector<T>& state, const std::unique_ptr<MeasParams>& params=std::make_unique<MeasParams>()) { 
        if (!params) {
            throw BadParams("Range and Bearing requires parameters.");
        }
        if (!utilities::instanceof<RangeBearingParams>(params.get())) {
